  ...
  async function sendWeeklyDigests() {
    ...
    for (const [memberId, memberItems] of Object.entries(byMember)) {
      ...
      const itemLines = memberItems.map(item => {
        const st = STATUS(item);
        const emoji = st === "out" ? "🚨" : "⚠️";
        return `${emoji} **${item.name}** — ${st.toUpperCase()} (${item.quantity}/${item.lowStockThreshold} ${item.unit})${item.supplier ? ` · ${item.supplier}` : ""}`;
      }).join("\n");
      ...
      if (member?.email && emailjsServiceId && emailjsTemplateId && emailjsPublicKey) {
        const plainItemLines = memberItems.map(item => {
          const st = STATUS(item);
          return `• ${item.name} — ${st.toUpperCase()} (${item.quantity}/${item.lowStockThreshold} ${item.unit})${item.supplier ? ` · Supplier: ${item.supplier}` : ""}`;
        }).join("\n");
        const params = {
          to_name: member.name,
          to_email: member.email,
