...
export default function App() {
  ...
  async function saveMember() {
    ...
    if (editingMemberId) {
      const updated = members.map(m => m.id === editingMemberId ? { ...m, name, discordId, email } : m);
      setMembers(updated);
      await storageSave("discord_members", JSON.stringify(updated));
  ...
  const initializedRef = useRef(false); // true after first load populates prevStatusRef

  useEffect(() => {
    if (!storageReady || !alertsLoaded) return;

    // On the very first run, just populate prevStatusRef silently — no alerts.
    // This prevents firing on page load for items that were already low before.
    if (!initializedRef.current) {
      items.forEach(item => {
        prevStatusRef.current[item.id] = STATUS(item);
      });
    ...
    items.forEach(item => {
      ...
      // Only alert on an actual transition into low/out, not on load
      if (isAlert && prev !== undefined && st !== prev && !alertedIds.has(alertKey)) {
  ...
  }, [items, storageReady, alertsLoaded, webhookUrl]);
  ...
  const [rawQty, setRawQty] = useState({}); // holds string while user is typing
  ...
  function setQty(id) {
    return (val) => {
      // Always store raw string so user can clear and retype
      setRawQty(p => ({...p, [id]: val}));
      const n = parseFloat(val);
      if (!isNaN(n) && n >= 0) {
        setPendingQty(p => ({...p, [id]: n}));
      } else {
        // Mark as pending with current value so confirm bar appears
        setPendingQty(p => ({...p, [id]: p.hasOwnProperty(id) ? p[id] : 0}));
      }
    };
  }

  function blurQty(id, fallback) {
    // On blur, if raw is empty or invalid, reset to original value
    const raw = rawQty[id];
    const n = parseFloat(raw);
    if (raw === undefined) return;
    if (isNaN(n) || n < 0) {
      // Discard — revert to committed value
      setRawQty(p => { const q = {...p}; delete q[id]; return q; });
      setPendingQty(p => { const q = {...p}; delete q[id]; return q; });
    } else {
      setRawQty(p => { const q = {...p}; delete q[id]; return q; });
    }
  }
  ...
  function commitQty(id) {
    ...
    setRawQty(p => { const n = {...p}; delete n[id]; return n; });
  }
    ...
    <div style={{minHeight:"100vh", background:"#0F0F0F", fontFamily:"'Courier New', Courier, monospace", color:"#E8E0D0"}}>
      ...
      <div className="top-bar" style={{background:"#1A1A1A", borderBottom:"2px solid #333", padding:"16px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12}}>
        ...
        <div className="header-actions" style={{display:"flex", gap:10, alignItems:"center"}}>
          {/* Sync indicator */}
          <div style={{display:"flex", alignItems:"center", gap:5, fontSize:11, color: syncing ? "#F0A500" : "#333", letterSpacing:"0.06em"}}>
          ...
          {lowCount > 0 && (
            <div style={{background:"#FF2D2D22", border:"1px solid #FF2D2D", borderRadius:4, padding:"4px 10px", fontSize:13, color:"#FF6B6B", fontWeight:700, cursor:"pointer"}} onClick={() => setFilterStatus(filterStatus === "low" ? "all" : "low")}>
              ⚠ {lowCount} LOW / OUT
            </div>
          )}
          <button onClick={() => requireAuth(openNew)} style={{background:"#F0A500", color:"#000", border:"none", borderRadius:4, padding:"8px 16px", fontSize:13, fontWeight:700, letterSpacing:"0.08em", cursor:"pointer", textTransform:"uppercase"}}>
            + ADD ITEM
          </button>
          {/* Lock indicator */}
          <div style={{display:"flex", alignItems:"center", gap:6}}>
            ...
            <button onClick={() => requireAuth(() => { setShowSettings(true); isEditingRef.current = true; })} title="Notifications & Settings" style={{background:"#222", border:"1px solid #444", borderRadius:4, padding:"8px 10px", fontSize:15, cursor:"pointer", lineHeight:1, position:"relative"}}>
              ⚙️
              {webhookUrl && <span style={{position:"absolute", top:4, right:4, width:6, height:6, borderRadius:"50%", background:"#2CB67D", display:"block"}} />}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="filter-bar" style={{background:"#141414", borderBottom:"1px solid #2A2A2A", padding:"12px 16px", display:"flex", gap:12, flexWrap:"wrap", alignItems:"center"}}>
        <input
          ...
          style={{background:"#222", border:"1px solid #333", borderRadius:4, padding:"7px 12px", fontSize:13, color:"#E8E0D0", width:"100%", maxWidth:280, outline:"none", fontFamily:"inherit"}}
        />
        <div className="filter-scroll" style={{display:"flex", gap:6, flexWrap:"wrap"}}>
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setFilterCat(c)} style={{
              ...
              borderRadius:3, padding:"4px 10px", fontSize:12, cursor:"pointer", fontFamily:"inherit", fontWeight: filterCat === c ? 700 : 400, letterSpacing:"0.05em"
            }}>{c}</button>
          ))}
        </div>
        <div className="sort-row" style={{display:"flex", gap:6, marginLeft:"auto", flexWrap:"wrap"}}>
          {["all","out","low","ok","good"].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)} style={{
              ...
              borderRadius:3, padding:"4px 10px", fontSize:12, cursor:"pointer", fontFamily:"inherit", textTransform:"uppercase", letterSpacing:"0.06em"
            }}>{s}</button>
          ))}
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{background:"#222", color:"#888", border:"1px solid #333", borderRadius:3, padding:"4px 8px", fontSize:12, fontFamily:"inherit", marginLeft:8}}>
      ...
      <div className="item-grid" style={{padding:"20px 24px", display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(300px, 1fr))", gap:12}}>
        {filtered.length === 0 && (
          <div style={{gridColumn:"1/-1", textAlign:"center", color:"#444", padding:60, fontSize:14, letterSpacing:"0.1em"}}>NO ITEMS MATCH YOUR FILTERS</div>
        )}
        {filtered.map(item => {
          const st = STATUS(item);
            ...
            <div key={item.id} style={{background:"#1A1A1A", border:"1px solid #2A2A2A", borderLeft:`3px solid ${stStyle.bg}`, borderRadius:4, padding:16, position:"relative"}}>
              ...
              <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10}}>
                <div style={{flex:1, paddingRight:8}}>
                  <div style={{fontSize:14, fontWeight:700, color:"#E8E0D0", lineHeight:1.3}}>{item.name}</div>
                  <div style={{fontSize:11, color:"#555", marginTop:3, letterSpacing:"0.08em"}}>{item.category}</div>
                </div>
                <div style={{display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6}}>
                  <span style={{background:stStyle.bg, color:stStyle.text, fontSize:11, fontWeight:700, padding:"2px 7px", borderRadius:2, letterSpacing:"0.1em"}}>{stStyle.label}</span>
                  <div style={{display:"flex", gap:4}}>
                    <button onClick={() => requireAuth(() => openEdit(item))} style={{background:"#222", border:"1px solid #333", borderRadius:3, color:"#888", fontSize:12, cursor:"pointer", padding:"2px 7px", fontFamily:"inherit"}}>EDIT</button>
                    <button onClick={() => requireAuth(() => setConfirmDelete(item.id))} style={{background:"#222", border:"1px solid #333", borderRadius:3, color:"#555", fontSize:12, cursor:"pointer", padding:"2px 7px", fontFamily:"inherit"}}>✕</button>
                  </div>
                </div>
              </div>
                ...
                return (<>
                  <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:10}}>
                    ...
                    <div style={{display:"flex", alignItems:"baseline", gap:4, flex:1, justifyContent:"center"}}>
                      <input
                        type="number"
                        value={rawQty.hasOwnProperty(item.id) ? rawQty[item.id] : displayQty}
                        onChange={e => requireAuth(() => setQty(item.id)(e.target.value))}
                        onBlur={() => blurQty(item.id, item.quantity)}
                      ...
                      />
                      <span style={{color:"#555", fontSize:12}}>{item.unit}</span>
                    </div>
                    <button className="qty-btn" onClick={() => requireAuth(() => adjustQty(item.id, item.quantity)(1))} style={{background:"#222", border:"1px solid #333", borderRadius:3, color:"#E8E0D0", width:28, height:28, cursor:"pointer", fontSize:16, lineHeight:1, fontFamily:"inherit"}}>+</button>
                  </div>
                  ...
                  {hasPending && (
                    <div style={{display:"flex", gap:6, marginBottom:10, alignItems:"center"}}>
                      ...
                      <button onClick={() => discardQty(item.id)} style={{background:"#222", border:"1px solid #333", borderRadius:3, color:"#666", fontSize:11, cursor:"pointer", padding:"3px 9px", fontFamily:"inherit", letterSpacing:"0.06em"}}>
                        DISCARD
                      </button>
                      <button onClick={() => requireAuth(() => commitQty(item.id))} style={{background:"#2CB67D", border:"none", borderRadius:3, color:"#fff", fontSize:11, fontWeight:700, cursor:"pointer", padding:"3px 12px", fontFamily:"inherit", letterSpacing:"0.06em"}}>
                        ✓ CONFIRM
                      </button>
                    </div>
                  )}
              ...
              {item.notes && <div style={{fontSize:11, color:"#444", marginTop:5, fontStyle:"italic"}}>{item.notes}</div>}
              {(() => { const m = members.find(mb => mb.id === item.assignedMemberId); return m ? (
                <div style={{fontSize:10, marginTop:6, display:"flex", alignItems:"center", gap:5, flexWrap:"wrap"}}>
      ...
      {/* Footer stats */}
      <div className="footer-stats" style={{padding:"12px 16px", borderTop:"1px solid #1E1E1E", display:"flex", gap:20, fontSize:12, color:"#444"}}>
        ...
        <div className="modal-wrap" style={{position:"fixed", inset:0, background:"#000000BB", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, padding:20}}>
          <div className="modal-inner" style={{background:"#1A1A1A", border:"1px solid #333", borderRadius:6, width:"100%", maxWidth:480, maxHeight:"90vh", overflowY:"auto"}}>
            <div style={{padding:"16px 20px", borderBottom:"1px solid #2A2A2A", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
              <span style={{fontSize:14, fontWeight:700, letterSpacing:"0.1em"}}>{editingId ? "EDIT ITEM" : "ADD NEW ITEM"}</span>
            ...
            <div style={{padding:20, display:"flex", flexDirection:"column", gap:14}}>
              {[
                ...
                <label key={key} style={{fontSize:12, color:"#666", letterSpacing:"0.08em"}}>
                  {label}
                  <input
                    type={type}
                    value={form[key]}
                    ...
                    placeholder={ph}
                    style={{display:"block", marginTop:5, width:"100%", background:"#111", border:"1px solid #333", borderRadius:3, padding:"8px 10px", fontSize:13, color:"#E8E0D0", fontFamily:"inherit", outline:"none", boxSizing:"border-box"}}
                  />
                </label>
              ))}
              <label style={{fontSize:12, color:"#666", letterSpacing:"0.08em"}}>
                Category
                <select value={form.category} onChange={e => setForm(f => ({...f, category: e.target.value}))} style={{display:"block", marginTop:5, width:"100%", background:"#111", border:"1px solid #333", borderRadius:3, padding:"8px 10px", fontSize:13, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}}>
                  {CATEGORIES.filter(c => c !== "All").map(c => <option key={c}>{c}</option>)}
                </select>
              </label>
              <div style={{display:"flex", gap:12}}>
                <label style={{fontSize:12, color:"#666", letterSpacing:"0.08em", flex:1}}>
                  Current Quantity
                  <input type="number" min="0" value={form.quantity} onChange={e => setForm(f => ({...f, quantity: e.target.value === "" ? "" : parseFloat(e.target.value) || 0}))} onBlur={e => setForm(f => ({...f, quantity: parseFloat(e.target.value) || 0}))}
                    style={{display:"block", marginTop:5, width:"100%", background:"#111", border:"1px solid #333", borderRadius:3, padding:"8px 10px", fontSize:13, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}} />
                </label>
                <label style={{fontSize:12, color:"#666", letterSpacing:"0.08em", flex:1}}>
                  Unit
                  <select value={form.unit} onChange={e => setForm(f => ({...f, unit: e.target.value}))} style={{display:"block", marginTop:5, width:"100%", background:"#111", border:"1px solid #333", borderRadius:3, padding:"8px 10px", fontSize:13, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}}>
              ...
              <label style={{fontSize:12, color:"#666", letterSpacing:"0.08em"}}>
                ...
                <input type="number" min="0" value={form.lowStockThreshold} onChange={e => setForm(f => ({...f, lowStockThreshold: e.target.value === "" ? "" : parseFloat(e.target.value) || 1}))} onBlur={e => setForm(f => ({...f, lowStockThreshold: parseFloat(e.target.value) || 1}))}
                  style={{display:"block", marginTop:5, width:"100%", background:"#111", border:"1px solid #333", borderRadius:3, padding:"8px 10px", fontSize:13, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}} />
              </label>
              <label style={{fontSize:12, color:"#666", letterSpacing:"0.08em"}}>
                Alert Recipient
                {members.length === 0 ? (
                  <div style={{marginTop:5, fontSize:12, color:"#444", padding:"8px 10px", background:"#111", border:"1px solid #222", borderRadius:3}}>
              ...
              <div style={{display:"flex", gap:10, marginTop:4}}>
                <button onClick={saveForm} disabled={!form.name.trim()} style={{flex:1, background:"#F0A500", color:"#000", border:"none", borderRadius:4, padding:"10px", fontSize:13, fontWeight:700, letterSpacing:"0.08em", cursor:"pointer", textTransform:"uppercase", opacity: form.name.trim() ? 1 : 0.4}}>
                  {editingId ? "SAVE CHANGES" : "ADD ITEM"}
                </button>
                <button onClick={closeForm} style={{background:"#222", color:"#888", border:"1px solid #333", borderRadius:4, padding:"10px 16px", fontSize:13, cursor:"pointer", fontFamily:"inherit"}}>
                  CANCEL
                </button>
              </div>
            </div>
      ...
      {confirmDelete && (
        <div style={{position:"fixed", inset:0, background:"#000000BB", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100}}>
          <div style={{background:"#1A1A1A", border:"1px solid #333", borderRadius:6, padding:24, maxWidth:320, textAlign:"center"}}>
            <div style={{fontSize:14, marginBottom:8, fontWeight:700}}>DELETE ITEM?</div>
            <div style={{fontSize:13, color:"#666", marginBottom:20}}>This cannot be undone.</div>
            <div style={{display:"flex", gap:10, justifyContent:"center"}}>
              <button onClick={() => deleteItem(confirmDelete)} style={{background:"#FF2D2D", color:"#fff", border:"none", borderRadius:4, padding:"8px 20px", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit"}}>DELETE</button>
              <button onClick={() => setConfirmDelete(null)} style={{background:"#222", color:"#888", border:"1px solid #333", borderRadius:4, padding:"8px 20px", fontSize:13, cursor:"pointer", fontFamily:"inherit"}}>CANCEL</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <div className="modal-wrap" style={{position:"fixed", inset:0, background:"#000000BB", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, padding:20}}>
          <div className="modal-inner" style={{background:"#1A1A1A", border:"1px solid #333", borderRadius:6, width:"100%", maxWidth:480, maxHeight:"90vh", overflowY:"auto"}}>
            <div style={{padding:"16px 20px", borderBottom:"1px solid #2A2A2A", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
              <div>
                <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", width:"100%"}}>
                  <div>
                    <span style={{fontSize:14, fontWeight:700, letterSpacing:"0.1em"}}>NOTIFICATIONS & SETTINGS</span>
                    <div style={{fontSize:11, color:"#555", marginTop:3}}>Discord & email alerts — fire automatically when stock drops low or runs out</div>
                  </div>
                  <button onClick={() => { setShowSettings(false); isEditingRef.current = false; setSettingPassword(true); }} style={{fontSize:11, color:"#555", background:"none", border:"1px solid #333", borderRadius:3, padding:"4px 8px", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>🔑 Password</button>
                </div>
              </div>
            ...
            <div style={{padding:20, display:"flex", flexDirection:"column", gap:14}}>
              <div style={{background:"#111", border:"1px solid #2A2A2A", borderRadius:4, padding:12, fontSize:12, color:"#666", lineHeight:1.7}}>
              ...
              {webhookUrl && (
                <div style={{fontSize:12, color:"#2CB67D", display:"flex", alignItems:"center", gap:6}}>
                  <span>●</span> Webhook active — alerts will fire when items drop to LOW or OUT
                </div>
              )}
              <div style={{display:"flex", gap:10}}>
                <button onClick={saveWebhook} disabled={!webhookInput.trim()} style={{flex:1, background:"#5865F2", color:"#fff", border:"none", borderRadius:4, padding:"10px", fontSize:13, fontWeight:700, letterSpacing:"0.08em", cursor:"pointer", textTransform:"uppercase", opacity: webhookInput.trim() ? 1 : 0.4}}>
                  SAVE WEBHOOK
                </button>
                {webhookUrl && (
                  <button onClick={async () => { await storageSave("discord_webhook", ""); setWebhookUrl(""); setWebhookInput(""); addToast("Webhook removed."); }} style={{background:"#222", color:"#888", border:"1px solid #333", borderRadius:4, padding:"10px 14px", fontSize:12, cursor:"pointer", fontFamily:"inherit"}}>
                    REMOVE
                  </button>
                )}
              </div>
              ...
              <div style={{borderTop:"1px solid #2A2A2A", paddingTop:14, marginTop:2}}>
                <div style={{fontSize:12, fontWeight:700, color:"#888", letterSpacing:"0.1em", marginBottom:6}}>EMAIL NOTIFICATIONS <span style={{fontSize:10, color:"#555", fontWeight:400}}>(via EmailJS)</span></div>
                ...
                <div style={{display:"flex", flexDirection:"column", gap:6}}>
                  {[["Service ID", "serviceId"], ["Template ID", "templateId"], ["Public Key", "publicKey"]].map(([label, key]) => (
                    <div key={key} style={{display:"flex", gap:6, alignItems:"center"}}>
                      <span style={{fontSize:11, color:"#555", width:90, flexShrink:0}}>{label}</span>
                      <input
                        placeholder={`EmailJS ${label}`}
                        value={emailjsInput[key] || ""}
                        onChange={e => setEmailjsInput(f => ({...f, [key]: e.target.value}))}
                        style={{flex:1, background:"#111", border:"1px solid #F0A50044", borderRadius:3, padding:"6px 9px", fontSize:12, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}}
                      />
                    </div>
                  ))}
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:4}}>
                    {emailjsServiceId && <span style={{fontSize:11, color:"#F0A500"}}>● Email active</span>}
                    {!emailjsServiceId && <span/>}
                    <div style={{display:"flex", gap:6}}>
                      {emailjsServiceId && (
                        <button onClick={async () => { await storageSave("emailjs_config", JSON.stringify({serviceId:"",templateId:"",publicKey:""})); setEmailjsServiceId(""); setEmailjsTemplateId(""); setEmailjsPublicKey(""); setEmailjsInput({serviceId:"",templateId:"",publicKey:""}); addToast("EmailJS config removed."); }} style={{background:"#222", color:"#888", border:"1px solid #333", borderRadius:3, padding:"6px 10px", fontSize:12, cursor:"pointer", fontFamily:"inherit"}}>REMOVE</button>
                      )}
                      <button onClick={saveEmailjsConfig} disabled={!emailjsInput.serviceId?.trim() || !emailjsInput.templateId?.trim() || !emailjsInput.publicKey?.trim()} style={{background:"#F0A500", color:"#000", border:"none", borderRadius:3, padding:"6px 14px", fontSize:12, fontWeight:700, cursor:"pointer", opacity: (emailjsInput.serviceId?.trim() && emailjsInput.templateId?.trim() && emailjsInput.publicKey?.trim()) ? 1 : 0.4}}>
                        SAVE
                      </button>
                    </div>
                  </div>
              ...
              <div style={{borderTop:"1px solid #2A2A2A", paddingTop:14, marginTop:2}}>
                <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6}}>
                  <div style={{fontSize:12, fontWeight:700, color:"#888", letterSpacing:"0.1em"}}>TEAM MEMBERS</div>
                  <button
                    onClick={async () => { await sendWeeklyDigests(); }}
                    title="Send Monday digest now (for testing)"
                    style={{fontSize:11, color:"#555", background:"none", border:"1px solid #333", borderRadius:3, padding:"3px 8px", cursor:"pointer", fontFamily:"inherit"}}
                  >
                    ▶ Send Digest Now
                  </button>
                </div>
                  ...
                  <div style={{display:"flex", flexDirection:"column", gap:6, marginBottom:10}}>
                    {members.map(m => (
                      <div key={m.id} style={{display:"flex", alignItems:"center", justifyContent:"space-between", background:"#111", border:"1px solid #222", borderRadius:3, padding:"7px 10px"}}>
                        <div style={{flex:1}}>
                          <span style={{fontSize:13, color:"#E8E0D0"}}>{m.name}</span>
                          <div style={{display:"flex", gap:8, marginTop:3, flexWrap:"wrap"}}>
                            {m.discordId && <span style={{fontSize:10, color:"#5865F2", background:"#5865F211", border:"1px solid #5865F244", borderRadius:2, padding:"1px 5px"}}>Discord: {m.discordId}</span>}
                            {m.email && <span style={{fontSize:10, color:"#F0A500", background:"#F0A50011", border:"1px solid #F0A50044", borderRadius:2, padding:"1px 5px"}}>Email: {m.email}</span>}
                          </div>
                        </div>
                        <div style={{display:"flex", gap:4}}>
                          <button onClick={() => { setEditingMemberId(m.id); setMemberForm({ name: m.name, discordId: m.discordId || "", email: m.email || "" }); }} style={{background:"none", border:"1px solid #333", borderRadius:2, color:"#888", cursor:"pointer", fontSize:10, padding:"2px 7px"}}>EDIT</button>
                          <button onClick={() => deleteMember(m.id)} style={{background:"none", border:"none", color:"#555", cursor:"pointer", fontSize:14, padding:"0 4px"}}>✕</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div style={{display:"flex", flexDirection:"column", gap:6}}>
                  <div style={{display:"flex", gap:6}}>
                    <input
                      ...
                      style={{flex:1, background:"#111", border:"1px solid #333", borderRadius:3, padding:"7px 9px", fontSize:12, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}}
                    />
                  </div>
                  <div style={{display:"flex", gap:6}}>
                    <input
                      ...
                      style={{flex:1, background:"#111", border:"1px solid #5865F244", borderRadius:3, padding:"7px 9px", fontSize:12, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}}
                    />
                    <input
                      placeholder="Email address (optional)"
                      value={memberForm.email}
                      onChange={e => setMemberForm(f => ({...f, email: e.target.value}))}
                      style={{flex:1, background:"#111", border:"1px solid #F0A50044", borderRadius:3, padding:"7px 9px", fontSize:12, color:"#E8E0D0", fontFamily:"inherit", outline:"none"}}
                    />
                  </div>
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                    <span style={{fontSize:10, color:"#444"}}>At least one of Discord ID or Email is required.</span>
                    <div style={{display:"flex", gap:6}}>
                      {editingMemberId && (
                        <button onClick={() => { setEditingMemberId(null); setMemberForm({ name: "", discordId: "", email: "" }); }} style={{background:"none", border:"1px solid #333", borderRadius:3, padding:"7px 12px", fontSize:12, color:"#888", cursor:"pointer"}}>
                          CANCEL
                        </button>
                      )}
                      <button onClick={saveMember} disabled={!memberForm.name.trim() || (!memberForm.discordId.trim() && !memberForm.email.trim())} style={{background:"#5865F2", color:"#fff", border:"none", borderRadius:3, padding:"7px 16px", fontSize:12, fontWeight:700, cursor:"pointer", opacity: (memberForm.name.trim() && (memberForm.discordId.trim() || memberForm.email.trim())) ? 1 : 0.4}}>
      ...
      {showAuthModal && (
        <div style={{position:"fixed", inset:0, background:"#000000CC", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:20}}>
          <div style={{background:"#1A1A1A", border:"1px solid #333", borderRadius:6, width:"100%", maxWidth:360, padding:24}}>
            <div style={{fontSize:14, fontWeight:700, letterSpacing:"0.1em", marginBottom:6}}>🔒 UNLOCK EDITING</div>
            <div style={{fontSize:12, color:"#555", marginBottom:16}}>Enter your password to make changes.</div>
            <input
              autoFocus
              type="password"
              value={authInput}
              ...
              style={{display:"block", width:"100%", background:"#111", border:`1px solid ${authError ? "#FF2D2D" : "#333"}`, borderRadius:3, padding:"9px 12px", fontSize:14, color:"#E8E0D0", fontFamily:"inherit", outline:"none", boxSizing:"border-box", marginBottom:8}}
            />
            {authError && <div style={{fontSize:12, color:"#FF6B6B", marginBottom:10}}>Incorrect password. Try again.</div>}
            <div style={{display:"flex", gap:8, marginTop:4}}>
              <button onClick={submitAuth} style={{flex:1, background:"#F0A500", color:"#000", border:"none", borderRadius:4, padding:"9px", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit"}}>UNLOCK</button>
              <button onClick={() => { setShowAuthModal(false); setPendingAction(null); }} style={{background:"#222", color:"#888", border:"1px solid #333", borderRadius:4, padding:"9px 14px", fontSize:13, cursor:"pointer", fontFamily:"inherit"}}>CANCEL</button>
            </div>
          </div>
        </div>
      )}
      ...
      {settingPassword && (
        <div style={{position:"fixed", inset:0, background:"#000000CC", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:20}}>
          <div style={{background:"#1A1A1A", border:"1px solid #333", borderRadius:6, width:"100%", maxWidth:360, padding:24}}>
            <div style={{fontSize:14, fontWeight:700, letterSpacing:"0.1em", marginBottom:6}}>{hasPassword ? "CHANGE PASSWORD" : "SET A PASSWORD"}</div>
            <div style={{fontSize:12, color:"#555", marginBottom:16}}>This password will be required to edit inventory.</div>
            <input
              autoFocus
              type="password"
              ...
              style={{display:"block", width:"100%", background:"#111", border:"1px solid #333", borderRadius:3, padding:"9px 12px", fontSize:14, color:"#E8E0D0", fontFamily:"inherit", outline:"none", boxSizing:"border-box", marginBottom:8}}
            />
            <input
              type="password"
              ...
              style={{display:"block", width:"100%", background:"#111", border:`1px solid ${passwordError ? "#FF2D2D" : "#333"}`, borderRadius:3, padding:"9px 12px", fontSize:14, color:"#E8E0D0", fontFamily:"inherit", outline:"none", boxSizing:"border-box", marginBottom:8}}
            />
            {passwordError && <div style={{fontSize:12, color:"#FF6B6B", marginBottom:8}}>{passwordError}</div>}
            <div style={{display:"flex", gap:8, marginTop:4}}>
              <button onClick={savePassword} style={{flex:1, background:"#F0A500", color:"#000", border:"none", borderRadius:4, padding:"9px", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit"}}>SAVE PASSWORD</button>
              <button onClick={() => { setSettingPassword(false); setNewPasswordInput(""); setNewPasswordConfirm(""); setPasswordError(""); }} style={{background:"#222", color:"#888", border:"1px solid #333", borderRadius:4, padding:"9px 14px", fontSize:13, cursor:"pointer", fontFamily:"inherit"}}>CANCEL</button>
            </div>
            {hasPassword && (
              <button onClick={() => { removePassword(); setSettingPassword(false); }} style={{marginTop:10, width:"100%", background:"none", border:"none", color:"#444", fontSize:12, cursor:"pointer", fontFamily:"inherit"}}>Remove password protection</button>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{position:"fixed", bottom:20, right:20, display:"flex", flexDirection:"column", gap:8, zIndex:200}}>
        {toasts.map(t => (
          <div key={t.id} style={{
            ...
            color:"#fff", borderRadius:4, padding:"10px 14px", fontSize:12, fontFamily:"'Courier New', monospace",
